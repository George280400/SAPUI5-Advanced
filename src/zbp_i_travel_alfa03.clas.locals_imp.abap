...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD get_instance_features.
    ...
    READ ENTITIES OF z_i_travel_alfa03
  ENTITY Travel
  ...
  cl_abap_context_info=>get_user_technical_name( ) EQ 'CB9980001007'
...
*Modify in local mode - BO - related updates there are not relevant for autorization objects
    ...
    MODIFY ENTITIES OF z_i_travel_alfa03 IN LOCAL MODE
    ENTITY Travel
    UPDATE FIELDS ( overall_status )
    WITH VALUE #( FOR key_row IN keys ( travel_id = key_row-travel_id overall_status = 'A' ) ) " Accepted
    FAILED failed
    REPORTED reported.
    READ ENTITIES OF z_i_travel_alfa03 IN LOCAL MODE
    ENTITY Travel
        FIELDS ( agency_id
                customer_id
                begin_date
                ...
                last_changed_by
                last_changed_at )
                WITH VALUE #( FOR key_row IN keys ( travel_id =
                key_row-travel_id ) )
  ...
  METHOD createTravelByTemplate.
    ...
    DATA lt_create_travel TYPE TABLE FOR CREATE z_i_travel_alfa03\\Travel.

    SELECT MAX( travel_id ) FROM ztravel_alfa03
    ...
    lt_create_travel = VALUE #( FOR create_row IN lt_read_entity_travel INDEX INTO idx
                                ...
                                booking_fee =
                                create_row-booking_fee
                                total_price =
                                create_row-total_price
                                currency_code =
                                create_row-currency_code
                                description = 'Add comments'
                                overall_status = 'O' ) ).

    MODIFY ENTITIES OF z_i_travel_alfa03
...
*Modify in local mode - BO - related updates there are not relevant for autorization objects
    ...
    ENTITY Travel
    UPDATE FIELDS ( overall_status )
    WITH VALUE #( FOR key_row IN keys ( travel_id = key_row-travel_id overall_status = 'X' ) ) " Reject
    FAILED failed
    REPORTED reported.
    READ ENTITIES OF z_i_travel_alfa03 IN LOCAL MODE
    ENTITY Travel
  ...
  METHOD validateCustomer.
    ...
    LOOP AT lt_travel ASSIGNING FIELD-SYMBOL(<ls_travel>).
      ...
      <ls_travel>-customer_id ] ).
        ...
        APPEND VALUE #( travel_id = <ls_travel>-travel_id %msg = new_message( id = 'Z_MC_TRAVEL_LOG'
                        number = '001'
  ...
  METHOD validateDates.
    ...
    READ ENTITY z_i_travel_alfa03\\Travel
  ...
  METHOD validateStatus.
    ...
    READ ENTITY z_i_travel_alfa03\\Travel FIELDS ( overall_status )
  WITH VALUE #( FOR <row_key> IN keys ( %key = <row_key>-%key ) )
  RESULT DATA(lt_travel_result).
    LOOP AT lt_travel_result INTO DATA(ls_travel_result).
...
CLASS lsc_Z_I_TRAVEL_ALFA03 DEFINITION INHERITING FROM cl_abap_behavior_saver.
...
CLASS lsc_Z_I_TRAVEL_ALFA03 IMPLEMENTATION.

  METHOD save_modified.

    DATA: lt_travel_log   TYPE STANDARD TABLE OF zlog_alfa03,
          lt_travel_log_u TYPE STANDARD TABLE OF zlog_alfa03.
    ...
    IF NOT create-travel IS INITIAL.
      ...
      LOOP AT lt_travel_log ASSIGNING FIELD-SYMBOL(<ls_travel_log>).
        ...
        <ls_travel_log>-changing_operation = lsc_z_i_travel_alfa03=>create.
    ...
    IF NOT update-travel IS INITIAL.
      ...
      LOOP AT update-travel INTO DATA(ls_update_travel).
        ...
        <ls_travel_log_bd>-changing_operation = lsc_z_i_travel_alfa03=>update.
    ...
    IF NOT delete-travel IS INITIAL.
      ...
      LOOP AT lt_travel_log ASSIGNING FIELD-SYMBOL(<ls_travel_log_del>).
        ...
        <ls_travel_log_del>-changing_operation = lsc_z_i_travel_alfa03=>delete.
    ...
    IF NOT lt_travel_log_u IS INITIAL.
      INSERT zlog_alfa03 FROM TABLE @lt_travel_log_u.
    ENDIF.
